...
await Homey.apps.getAppSettings({id: 'com.athom.homeyscript'})
  .then(result => {
    ...
    try {
      ...
      const realEntries = Object.entries(result.scripts).filter(([id, s]) => s && s.name && !s.name.startsWith('__mcp_run') && !UUID_RE.test(s.name));
      ...
      // Diagnostic (v1.5.3): the API call itself succeeded (we're in .then),
      // but processing its result threw — most likely result.scripts or
      // result.tokens isn't shaped the way it is on Homey Pro. Reported on
      // the forum as failing on Homey Self-Hosted ("Failed: Getting
      // HomeyScript"), but the app id itself was confirmed correct there,
      // and other apps' getAppSettings calls work fine on SHS — so this
      // logs the actual error and the top-level keys of `result` instead of
      // guessing at a fix, to find out what's actually different about the
      // SHS response shape.
