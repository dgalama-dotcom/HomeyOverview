...
await Homey.devices.getDevices()
  .then(result => {
    ...
    report.devices = {
      virtual, virtualNames: virtualNames.sort(),
      ir, irNames: irNames.sort(),
      other, otherNames: otherNames.sort(),
      unifiExcluded,
  ...
  if (report.zigbee && !report.zigbee.failed) {
    sections.push({
      ...
      html: `
        ...
        ${showZigbeeNodes ? '<h3>All Zigbee nodes</h3>' + ul(report.zigbee.nodes) : ''}
