  ...
  await Homey.system.getStorageInfo()
    .then(result => {
      ...
      if (result && typeof result.total === 'number' && typeof result.free === 'number') {
        ...
        log('Storage: unexpected result shape —', result ? JSON.stringify(result) : 'no result');
      }
    })
    .catch(err => {
      ...
      log('Failed: Getting storage information —', (err && err.message) || err);
    });
}
...
await Homey.zigbee.getState()
  .then(result => {
    ...
    // type above).
    let staleCount = 0, staleNodeNames = [];
    ...
    Object.keys(result.nodes).forEach(function(key) {
      ...
      if (typeof node.lastSeen === 'number' && (zigbeeNowMs - node.lastSeen) > staleThresholdMs) {
        staleCount++; staleNodeNames.push(node.name);
      }
    });
...
if (previousSnapshot) {
  ...
  if (report.zigbee && !report.zigbee.failed) {
    ...
    diffLines('📶 Zigbee nodes newly not responding (>' + zigbeeStaleDays + ' days)', '📶 Zigbee nodes reporting again', previousSnapshot.zigbeeStale, report.zigbee.stale, previousSnapshot.zigbeeStaleNodes, report.zigbee.staleNodes)
      .forEach(e => findings.push({ level: e.tag === 'added' ? 'warn' : 'info', ...e }));
  }
} else {
