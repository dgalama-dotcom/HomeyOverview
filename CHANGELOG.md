...
- **Footer text: include the script's own name, and use a smaller,
  ...
  generated by HomeyScript v1.6.0." It should also name the script
  itself, e.g. "Automatically generated by HomeyScript homey-overview
  v1.6.0." — read dynamically so that if the script is ever renamed
  (currently `homey-overview`), the footer follows automatically
  without needing a manual edit. Needs a quick check of whether
  HomeyScript exposes a running script's own name to itself at
  runtime (e.g. via the `Homey` object); if not, fall back to a
  manually-set `SCRIPT_NAME` const near `VERSION` at the top, with a
  comment reminding you to update it if you rename the script.
  Styling: wrap the footer line in smaller, italic text (e.g. reduce
