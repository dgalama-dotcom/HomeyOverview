...
1. **A Homey ;-)** (Homey Pro, or Homey Self-Hosted). Some system-health
   fields (backup, storage, throttling, images, group devices) only
   populate on Homey Pro models from 2023 onward.
2. The **HomeyScript** app installed (App Store, search "HomeyScript").
3. An **email-sending app** installed on Homey that provides a flow
   ...
   fields will work; if you use a different one, see "Using a
   different email app" below.
4. An email client isn't required on your end: the script sends the
...
7. Just below that, find:

   ```js
   const staleBackupWarningDays = 7;
   ```

   This controls the Summary's stale-backup warning (e.g. "⚠️ Last
   backup is more than 7 days old"), which fires whenever your last
   successful backup is older than this many days. Unlike most of the
   Summary, this isn't a scan-to-scan diff, so it keeps firing on every
...
disabled apps, Stable-channel apps, Infrared devices, broken flows,
Z-Wave battery devices, Zigbee router devices) is included in the
email, or only counted. Set any of them to `false` if you want a
