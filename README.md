...
"Automatically generated by HomeyScript v1.5.6."), so you can tell at
...
## v1.4.1 — consistent alphabetical sorting

All name lists are now sorted alphabetically, consistently. Previously
only Devices (Virtual/IR/Other) and the Z-Wave subcategories were
sorted; Apps, Flows, the main Zigbee node list, Logic variables,
HomeyScript scripts, Better Logic variables, Moods, Users, Group
devices, and the main "All Z-Wave devices" list were not. This also
affects the order of names shown in the Summary's added/removed
findings.

Also, based on forum feedback: comments that referred to
"Homey Pro 2023" as if it were a distinct model have been reworded to
"Homey Pro models post-2022" — the Pro Mini and later models share the
same platform/firmware as the original 2023 model, so "2023" wasn't an
accurate way to describe which models get backup/storage/throttling/
images/group-devices support. This is a documentation-only change; the
underlying code check was already model-agnostic.

## v1.5.0 — firmware/name changes, stale-backup warning, more diffs

Seven small gaps in the Summary are closed:

- **Homey firmware version changes are now reported.** The current
  firmware version was already shown in the System table, but an
  actual update between two scans never showed up in the Summary —
  you'd only notice by reading the full report and remembering what
  it used to say. Now shown as, e.g., "🏠 Homey firmware updated:
  12.4.5 → 12.5.0." This is separate from the existing "Update
  available" line, which reports an update *waiting to be installed*.
- **Homey name changes are now reported**, e.g. "🏠 Homey renamed:
  Smart Home Hub → New Name."
- **New stale-backup warning**, via the `staleBackupWarningDays`
  setting (default `7` — see Installation step 7 above). Unlike
  almost everything else in the Summary, this isn't a scan-to-scan
  diff: it's a threshold check against your *current* backup age, so
  it keeps warning on every run until a fresh backup completes, e.g.
  "⚠️ Last backup is more than 7 days old."
- **Images count is now diffed**, e.g. "🖼️ Images: 3 → 5 (+2)."
- **Alarms are now diffed** — both the total count and how many are
  enabled.
- **Moods are now diffed by name**, not just counted — the same gap
  the v1.4.0 Logic variables fix addressed for that category.
- **Zones are now diffed by name**, not just counted — previously
  "📍 Zones: 8 → 9" didn't say which zone changed.

## v1.5.1 — Homey Self-Hosted WiFi/Ethernet fix

Fixes a false alarm reported on the forum: on Homey Self-Hosted (SHS),
every run showed "⚠️ WiFi/Ethernet disconnected" in the Summary, even
though the report had just emailed successfully. SHS runs via the host
machine's own network stack, so it has no separate WiFi/Ethernet radios
to report on — those fields now show "-" on SHS instead of firing a
false disconnect warning. Detected via the Homey model name; no setting
to change, this applies automatically. Storage, throttling, backup and
other system-health checks are unaffected.

## v1.5.2 — Homey Self-Hosted backup-status fix

Fixes a second SHS issue from the same forum report: the "Last backup"
check failed on every run ("Failed: Getting last backup" in the
tester's log), because it's gated behind the same system flag SHS also
reports. Homey Self-Hosted doesn't support the backup-status API, so
"Last backup" now shows "-" there instead of a misleading "⚠️ No backup
found", and the stale-backup warning (added in v1.5.0) is no longer
falsely triggered on SHS either. Storage keeps working exactly as
before — this fix only excludes the backup check, nothing else. No
setting to change, this applies automatically.

## v1.5.3 — diagnostics for a remaining Homey Self-Hosted issue

No user-facing change. The HomeyScript settings check still fails on
SHS ("Failed: Getting HomeyScript" — see the CHANGELOG's "Planned for
a future release" section), but a forum tester ruled out a wrong app
id and confirmed a similar check for another app works fine on SHS.
So this version just logs the actual error message (and, if relevant,
what fields the API actually returned) to find the real cause on the
next test run, instead of guessing at a fix.

## v1.5.4 — Homey Self-Hosted HomeyScript-settings fix

Fixes the last of three Homey Self-Hosted (SHS) issues reported on the
forum. The v1.5.3 diagnostics found the exact cause: on SHS, the
HomeyScript app's settings object has no `tokens` field at all, and the
script crashed trying to count it — silently losing the whole result,
including the scripts list, which was actually available all along.
Fixed by treating a missing `tokens` field as zero. The HomeyScript
scripts section now works correctly on SHS, same as on Homey Pro. No
setting to change, this applies automatically. With this, all three
SHS issues from the forum report (WiFi/Ethernet, backup-status,
HomeyScript settings) are fixed.

## v1.5.5 — filter out Homey's built-in example scripts

Forum-requested: the HomeyScript scripts count/list now excludes
Homey's own built-in "example-xxx" scripts (example-hello-world,
example-fetch, etc.) by default, since they ship with every Homey and
aren't something you added. New setting near the top of the script,
`showExampleScripts` (default `false`), brings them back into the
count/list if you want them included.

## v1.5.6 — every category's items are now actually listed in the report

The report's own design intent is: a Summary of changes at the top, then
per category a line or two of counts, then every present item listed.
Several categories broke that intent — a `show...` toggle existed and
was respected for the console log, but the corresponding name list was
never actually rendered into the emailed HTML report, so those items
stayed invisible even with the toggle on. Fixed for: Apps (new
`showStableApps` toggle — Stable-channel apps had no name list at all
before), Devices (Infrared devices), Z-Wave (Router, Unsecure, Secure S0,
Secure S2 Authenticated, Secure S2 Unauthenticated device lists), and
Zigbee (Router, End device lists). No new data is collected — it was
already tracked internally for counts and the Summary diff — this just
surfaces it in the detail report too.
