# Homey Overview v1.3 — weekly system report by email
...
new users added, throttling, renamed devices, etc.), so you don't have
to read the whole report every time.

The current version number is shown in the email footer (e.g.
"Automatically generated by HomeyScript v1.3.0."), so you can tell at
...
Most changes are reported with the actual item name(s) as a bulleted
sub-list under the finding (e.g. "2 Basic flows created since previous
scan: Flow A, Flow B") rather than just a bare count. A few categories
(logic variables, zones, total device count) are count-only, since a
meaningful name list either isn't available or isn't worth the noise.

As of v1.3, "Other devices" and "Virtual devices" also distinguish a
genuine rename (e.g. "Dewi" → "Smartphone Dewi") from a real add/remove,
shown as its own "renamed" finding — see "What's new in v1.3" below.
...
**v1.2.0** (thanks to forum feedback from SingKT):
- Z-Wave "newly unreachable" and "unknown type" nodes now show WHICH
  nodes changed (a name/ID list), not just a bare count.
- Names in the Summary are now a proper bulleted sub-list under each
  finding, instead of one comma/and-joined sentence — easier to scan
  with more than a couple of items.
- Fixed a bug where turning off a "show...Devices/Apps/Flows" toggle
  also silently broke the Summary diff/snapshot for that category, and
  left a misleading "— none —" in the detail report for a section
  you'd deliberately hidden.
...
- Fixed a gap in the HomeyScript "real script" filter: ad-hoc/inline
  ...
  were slipping through and showing up in the report as a bare UUID.
- "Other devices" and "Virtual devices" are now diffed by Homey's
  stable device ID instead of by name, so a rename is reported as a
  single "renamed" finding instead of a misleading remove+add pair.
  (Falls back to the old name-only diff for one run right after
  upgrading, since no ID-based snapshot exists yet — after that, it's
  fully reliable.)
- Zigbee: removed the separate "Unknown type devices" section (it
  duplicated a subset of "All Zigbee nodes"). Unknown-type nodes are
  now annotated directly in the "All Zigbee nodes" list instead (e.g.
  "Name (unknown type)"), still controlled by the
  `showZigbeeUnknownDevices` toggle.
- Moved the script version from the email's top subtitle line to the
  footer ("Automatically generated by HomeyScript vX.X.X.").

See CHANGELOG.md for the full version history, including v1.1 and v1.0.
...
- HomeyScript scripts named `__mcp_run_*`, left unnamed, or named with
  a bare UUID are excluded from the script count — these are internal
  leftovers from certain AI/MCP bridge integrations, not real user
  scripts. Harmless if you don't use one.
