# Homey Overview v1.3.0 — weekly system report by email
...
As of v1.3.0, "Other devices" and "Virtual devices" also distinguish a
genuine rename (e.g. "Dewi" → "Smartphone Dewi") from a real
add/remove, shown as its own "renamed" finding — see "What's new in
v1.3.0" below.
...
## v1.3.0 — Reliable device renames, cleanup fixes

Prompted by a stray raw-UUID script entry and a device rename showing
up as a false remove+add:

- **Renamed devices are now recognized as renames**, not a false
  remove+add. "Other devices" and "Virtual devices" are diffed by
  Homey's stable device ID instead of by name, so renaming a device
  (e.g. "Dewi" → "Smartphone Dewi") is reported as a single "renamed"
  finding. Right after upgrading to v1.3.0, one run may still show the
  old add+remove behavior (no ID-based snapshot exists yet to compare
  against) — every run after that is fully reliable.
- **HomeyScript script count no longer picks up stray UUID-named
  entries.** Some ad-hoc/inline script runs (e.g. via an MCP bridge's
  "run this code directly" feature) get auto-named after their own
  script ID instead of being left unnamed, which slipped past the
  existing `__mcp_run_*`/unnamed-entry filters. Now excluded too.
- **Zigbee: removed the overlap between "All Zigbee nodes" and
  "Unknown type devices".** Unknown-type nodes are now annotated
  directly in the "All Zigbee nodes" list (e.g. "Homey Weerterstraat
  (unknown type)") instead of appearing in both lists. Still
  controlled by the `showZigbeeUnknownDevices` toggle.
- **Script version moved from the email's top subtitle line to the
  footer** ("Automatically generated by HomeyScript v1.3.0.").
