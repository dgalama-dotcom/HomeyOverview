...
const showStableApps = true; // Show names of Stable-channel apps (the majority — everything not Test/Development)
...
const showZigbeeStaleNodes = true; // Show names of Zigbee nodes that haven't reported in over zigbeeStaleDays (see below)
...
const showLowBatteryDevices = true; // Show names of devices at or below lowBatteryWarningPercent (see below)
...
// Optional: also send a short, plain-text version of the Summary & Actions
// section to WhatsApp, on every run, in addition to the HTML email above.
// Uses the community CallMeBot app (com.gruijter.callmebot) — install it
// from the App Store and add a WhatsApp device there first (you'll need a
// CallMeBot API key for that; see the app's own setup for how to get one).
// The device's phone number and API key live in THAT device's settings in
// Homey, not in this script, so nothing sensitive needs to be hardcoded
// here. This script finds your CallMeBot WhatsApp device automatically (by
// its driver, regardless of what you named it) and calls its own "Send a
// message" flow action — same pattern as the email send further down. See
// the README for setup steps.
const sendWhatsAppSummary = false; // Set to true to enable (after installing the CallMeBot app and adding a WhatsApp device — see README)
...
// Number of days of silence after which a Zigbee node is flagged in the
// Summary as "not responding" (based on that node's own last-seen
// timestamp on the Zigbee network) — the Zigbee equivalent of the
// existing Z-Wave "newly unreachable" check above. Like everything else
// in the Summary (other than the backup/battery threshold checks), this
// IS a scan-to-scan diff: it fires once when a node newly crosses the
// threshold, and again when it reports again. Some battery-powered
// end-devices sleep for hours between reports by design, so a very low
// value here can cause false positives — raise it if you see those.
const zigbeeStaleDays = 3;

// Battery percentage at or below which a device is flagged in the
// Summary as low on battery (once, when it newly crosses this
// threshold — same scan-to-scan diff pattern as most of the Summary).
// Applies to any device that reports a battery percentage
// (measure_battery), regardless of connection type (Zigbee, Z-Wave,
// Wi-Fi, etc.) — not just Z-Wave, which is all the existing
// showZwaveBatteryDevices toggle covers (and that only lists which
// devices run on battery at all, not their current level). Devices that
// only expose a plain low-battery flag (alarm_battery) instead of a
// percentage are included too, as a simple yes/no. Devices of class
// "car" are excluded — EVs commonly and normally sit below most
// reasonable percentages while just going about their day, which isn't
// a "replace the battery" situation the way it is for a sensor.
...
// The script's own name, so the email footer can say which script
// produced it even if you rename it later. Read dynamically from
// HomeyScript's own __filename__ global rather than hardcoded — confirmed
// (by testing a run triggered the same way a real flow triggers this
// script) that __filename__ resolves to "<script name>.js" when actually
// run via a flow action, not just some placeholder. Falls back to a
// generic label if that global is ever missing (defensive only — not
// expected to happen).
const SCRIPT_NAME = (typeof __filename__ === 'string' && __filename__.toLowerCase().endsWith('.js'))
  ? __filename__.slice(0, -3)
  : 'homeyscript';
...
//
// Added in v1.5.6:
//  - Closed a gap versus the report's own design intent (Summary → per-
//    category counts → every present item listed): several categories had
//    a show...-toggle that was respected for the CONSOLE log, but whose
//    name list was never actually rendered into the emailed HTML report
//    at all, so those items were invisible in the report itself. Fixed for:
...
//       Test/Development/disabled/crashed/updateable were named.
//     - Devices: Infrared devices — showIRDevices existed but its list
//       was console-only.
//     - Z-Wave: Router, Unsecure, Secure (S0), Secure (S2 Authenticated),
//       Secure (S2 Unauthenticated) device lists — all five toggles
//       existed but were console-only.
//     - Zigbee: Router, End device lists — both toggles existed but were
//       console-only.
//    No new data is collected — all of this was already being tracked
//    internally for counts and Summary diffs; this just also surfaces it
//    in the detail report, consistent with every other named category.
//
// Added in v1.5.7:
//  - Z-Wave summary line reworded: "unreachable" is a genuine subset of
//    the paired-device count and now reads that way ("... battery, 1
//    currently unreachable."); "unknown" node(s) are NOT part of that
//    count (they're raw node IDs in the Z-Wave network with no matching
//    paired device) and are now a separate sentence, instead of being
//    listed as if it were a sibling of the security/power breakdown.
//    Checked the rest of the script for the same issue: Zigbee's
//    equivalent line was already a true partition and needed no change.
//  - Z-Wave "Unreachable nodes" now shows the paired device's name where
//    one is known (e.g. "Lamp boom (Node 39)") instead of a bare "Node
//    39", in the detail report, the console log, and the Summary &
//    Actions diff (all three read from the same underlying list). The
//    name comes from Homey's own device registry, not from live
//    communication with the node, so it's available regardless of
//    whether the node is currently reachable. "Unknown nodes" is
//    unchanged and still shows bare node IDs, since those genuinely
//    have no matching paired device to name (confirmed against Homey's
//    own native Z-Wave node list, which shows the same nodes as
//    "Unknown Node" there too). Note: if a node was already unreachable
//    before updating to this version, the very next run may show a
//    one-time "39 reachable again" / "Lamp boom (Node 39) newly
//    unreachable" pair in the Summary, purely because the stored format
//    changed; from the run after that, the diff is accurate again.
//
// Added in v1.5.8:
//  - Forum/user-requested: optional WhatsApp notification. On every run, if
//    sendWhatsAppSummary is set to true, a plain-text version of the Summary
//    & Actions section is also sent via CallMeBot's free personal API,
//    alongside the existing HTML email. Only the Summary is sent (not the
//    full detail report) — WhatsApp isn't the place for a multi-section
//    report, and the whole point of a phone notification is to be short.
//    Uses HomeyScript's built-in fetch() to call CallMeBot directly; no new
//    Homey app dependency. Off by default, since it needs your own
//    CallMeBot phone number + API key filled in first (see the new
//    CALLMEBOT_PHONE/CALLMEBOT_APIKEY settings near the top, and the
//    README for the one-time CallMeBot registration steps). Sent
//    unconditionally on every run (not gated by onlyShowDetailsOnChange),
//    same as the email's Summary section always being included.
//
// Added in v1.6.0:
//  - Changed how the WhatsApp summary (v1.5.8) is sent: instead of a direct
//    HTTP call to CallMeBot with a phone number and API key hardcoded as
//    script consts, it now calls the community CallMeBot Homey app's own
//    "Send a message" flow action on your CallMeBot WhatsApp device — found
//    automatically by its driver ID, no device name or ID hardcoded either.
//    Same idea as how the email is sent (via a flow action card, not a raw
//    API call), and keeps your phone number/API key only in that device's
//    own Homey settings, never in the script text itself — safer for a
//    script you might share or paste elsewhere. Requires the CallMeBot app
//    (com.gruijter.callmebot) installed with a WhatsApp device added; see
//    the README. The CALLMEBOT_PHONE/CALLMEBOT_APIKEY consts from v1.5.8
//    are removed — nothing to fill in here anymore.
//
// Added in v1.7.0:
//  - Email footer now reads "Automatically generated by HomeyScript
//    <script-name> v1.7.0." — the script's own name (SCRIPT_NAME, read
//    dynamically from HomeyScript's __filename__ global, not hardcoded)
//    is included so you can tell which script produced a report even if
//    you rename it later. Footer text is also now a size smaller and
//    italic.
//  - Zigbee: nodes that haven't reported in over zigbeeStaleDays (default
//    3) are now flagged as "not responding" in the Summary, the same way
//    Z-Wave's "newly unreachable" check already worked — closes a gap
//    between the two sections.
//  - New low-battery warning: any device reporting a battery percentage
//    (measure_battery) at or below lowBatteryWarningPercent (default
//    20%), or a plain low-battery flag (alarm_battery) with no
//    percentage, is flagged once in the Summary when it newly crosses
//    that threshold — across all device types, not just Z-Wave (EVs are
//    excluded; see the lowBatteryWarningPercent comment above).
//  - New: an unexpected Homey restart (uptime shorter than the previous
//    scan recorded, with no firmware update to explain it) is now
//    flagged in the Summary — previously invisible unless it happened to
//    coincide with a firmware-update notice.
//
// Added in v1.7.1:
//  - Zigbee and Z-Wave devices are now diffed by Homey's stable ID
//    (`ieeeAddress` for Zigbee, `device.id` for Z-Wave — same pattern
//    Virtual/Other devices already used), instead of by name only. A
//    device renamed in the Homey app is now reported as a single
//    "renamed" finding (e.g. "📶 Zigbee devices renamed: Old name →
//    New name") instead of a confusing remove+add pair — this also
//    fixes a false remove+add pair from a single stray space in an
//    otherwise-identical name. Same one-run fallback to the old
//    name-only diff as Virtual/Other devices got when this was
//    introduced for them, for the first run after upgrading (no
//    ID-based snapshot yet). A genuine hardware swap (new
//    `ieeeAddress`/`device.id`) still correctly shows as remove+add,
//    since that's a different physical device even with the same name.
//  - Diagnostics added for the Storage row: a single occurrence was
//    seen where Storage showed "-" while sibling post-2022 fields
//    (backup, images) reported fine in the same run — cause unknown.
//    The storage call now logs the actual error message on failure,
//    and the raw result shape if it resolves without the expected
//    `total`/`free` fields, instead of a bare "Failed" message with no
//    detail — so a recurrence can actually be diagnosed. No behavior
//    change if it doesn't recur.
//
// Added in v1.7.2:
//  - Zigbee "not responding"/"reporting again" and Z-Wave "unreachable"/
//    "reachable again" tracking are now diffed by stable ID (Zigbee
//    ieeeAddress, Z-Wave protocol node number) instead of by name/display
//    string. Found after the v1.7.1 rename fix: renaming a node while it
//    was already stale/unreachable produced a false "newly not
//    responding"/"newly unreachable" + "reporting/reachable again" pair,
//    since only the ADDED/REMOVED/RENAMED diff had moved to ID-based
//    comparison — this subset check hadn't. Also now skips reporting a
//    "recovered" finding for an ID that no longer exists at all (deleted),
//    since that's already covered once by the added/removed/renamed diff.
//  - Basic and Advanced flows are now diffed by Homey's stable flow `id`
//    for created/deleted/renamed (same `diffByIdWithRename` pattern as
//    devices), instead of by name only — a renamed flow is now reported
//    as "renamed" instead of a remove+add pair. The broken/disabled
//    subset checks are also now ID-based and fully independent of the
//    rename diff, so a flow that's renamed AND has a broken/disabled
//    status change in the same run correctly produces two separate
//    findings (e.g. "Advanced flows renamed: A → B" AND "Advanced flows
//    broken: B"), always shown under its current name.
//  - The Storage-bug diagnostics (added in v1.7.1) are now also written
//    to a dedicated Logic variable ("Overview_storage_diagnostic"),
//    persisting the most recent failure/anomaly (timestamp + detail)
//    instead of only a HomeyScript console log() call. Found that a
//    flow-triggered run's console output isn't retrievable after the
//    fact at all, which made the v1.7.1 diagnostics useless in practice
//    (confirmed during an actual 3-day recurrence, 17-19 September,
//    where the console log from those runs was already gone by the time
//    anyone looked). The new variable can be read at any later point,
//    same as "Overview_previous_snapshot" already works.
...
// -------- Fetch/create storage-diagnostic variable (added in v1.7.2) --------
// Persists the LAST storage-call failure/anomaly (timestamp + detail) in a
// dedicated Logic variable, since HomeyScript's console log from a
// flow-triggered run turned out to not be retrievable after the fact (found
// while investigating an intermittent "Storage: -" report — the v1.7.1
// log()-only diagnostics were already gone by the time anyone could check).
// Only ever OVERWRITTEN on a new failure/anomaly below, so it keeps showing
// the most recent problem until the next one replaces it — read it any time
// via Homey.logic.getVariables() (or the MCP logic_get/logic_list tools), no
// need to catch it live. Same create-if-missing pattern as the snapshot
// variable above.
let storageDiagVarId = null;
try {
  const varsForStorageDiag = await Homey.logic.getVariables();
  const diagVar = Object.values(varsForStorageDiag).find(v => v.name === 'Overview_storage_diagnostic');
  if (diagVar) {
    storageDiagVarId = diagVar.id;
  } else {
    const createdDiag = await Homey.logic.createVariable({ variable: { name: 'Overview_storage_diagnostic', type: 'string', value: '' } });
    storageDiagVarId = createdDiag.id;
  }
} catch (e) { /* couldn't read or create the storage-diagnostic variable — a failure this run just won't be persisted */ }
...
await Homey.system.getInfo()
  .then(result => {
    ...
    // Raw seconds, kept alongside the formatted uptimeText above so the
    // next run can detect an unexpected restart (see the Summary section
    // further down) — uptime can only increase while Homey keeps
    // running, so a smaller value next time means it restarted somewhere
    // in between.
    report.uptimeSeconds = result.uptime;
...
if (showStorage && homeyPlatformVersion === 2) {
  await Homey.system.getStorageInfo()
    .then(result => {
      // Diagnostics added in v1.7.1: a single occurrence was observed
      // (Homey Pro Early 2023, firmware 13.5.0) where Storage showed "-"
      // in the report while sibling post-2022 calls (backup, images)
      // succeeded in that same run — cause unknown. This surfaces the
      // actual result shape if it's ever missing the expected fields,
      // instead of silently just not setting report.storageTotal/Free
      // (same diagnostic pattern as the v1.5.3 getAppSettings logging).
      if (result && typeof result.total === 'number' && typeof result.free === 'number') {
        report.storageTotal = result.total;
        report.storageFree = result.free;
        log('Storage:', result.total, 'total,', result.free, 'free');
      } else {
        const detail = 'Unexpected result shape — ' + (result ? JSON.stringify(result) : 'no result');
        log('Storage:', detail);
        // Persisted to a Logic variable in v1.7.2 (see comment above) — a
        // bare log() call turned out to be useless for a flow-triggered
        // run, since its console output isn't retrievable after the fact.
        if (storageDiagVarId) {
          Homey.logic.updateVariable({ id: storageDiagVarId, variable: { type: 'string', value: JSON.stringify({ ts: new Date().toISOString(), issue: detail }) } }).catch(() => {});
        }
      }
    })
    .catch(err => {
      // Diagnostics added in v1.7.1 (see comment above) — logs the actual
      // error instead of a bare "Failed" message, so a recurrence can
      // actually be diagnosed next time.
      const detail = 'Failed: Getting storage information — ' + ((err && err.message) || err);
      log(detail);
      // Persisted to a Logic variable in v1.7.2 — see comment above.
      if (storageDiagVarId) {
        Homey.logic.updateVariable({ id: storageDiagVarId, variable: { type: 'string', value: JSON.stringify({ ts: new Date().toISOString(), issue: detail }) } }).catch(() => {});
      }
    });
...
await Homey.flow.getFlows()
  .then(result => {
    ...
    const allNames = [];
    // Added in v1.7.2: id -> name map for every flow (rename detection,
    // same diffByIdWithRename pattern as devices), plus separate ID lists
    // for the broken/disabled subsets (state-transition detection that's
    // immune to a same-run rename — see diffIdSubset() further down).
    let allById = {}, brokenIds = [], disabledIds = [];
    Object.keys(result).forEach(function(key) {
      const flow = result[key];
      const id = flow.id || key;
      allNames.push(flow.name);
      allById[id] = flow.name;
      if (!flow.enabled) { disabled++; disabledNames.push(flow.name); disabledIds.push(id); }
      if (flow.broken) { broken++; brokenNames.push(flow.name); brokenIds.push(id); }
    });

    report.flows = {
      total: Object.keys(result).length, broken, disabled,
      allNames, allById,
      disabledNames: disabledNames.sort(), disabledIds,
      brokenNames: brokenNames.sort(), brokenIds
    };
...
await Homey.flow.getAdvancedFlows()
  .then(result => {
    let disabled = 0, broken = 0, disabledNames = [], brokenNames = [];
    const allNames = [];
    // Added in v1.7.2 — same as Basic flows above.
    let allById = {}, brokenIds = [], disabledIds = [];
    Object.keys(result).forEach(function(key) {
      const flow = result[key];
      const id = flow.id || key;
...
log('\r\n----------------- Devices -------------------');
let allDevices = 0, zwave = 0, zwaveDevices = [], zwaveNodes = [], zwaveNodeName = {}, zwaveRouter = 0, zwaveRouterDevices = [], zwaveBattery = 0, zwaveBatteryDevices = [], zwaveSx = 0, zwaveSxDevices = [], zwaveS0 = 0, zwaveS0Devices = [], zwaveS2Auth = 0, zwaveS2AuthDevices = [], zwaveS2Unauth = 0, zwaveS2UnauthDevices = [];
// device.id -> device.name map for Z-Wave devices, added in v1.7.1 — same
// pattern as virtualById/otherById, lets a rename be reported as a single
// "renamed" finding instead of a remove+add pair (see diffByIdWithRename()
// further down).
let zwaveById = {};
let lowBatteryCount = 0, lowBatteryDevices = [];
...
await Homey.devices.getDevices()
  .then(result => {
    ...
    Object.keys(result).forEach(function(key) {
      ...
      // Low battery: checked here, independent of the category the device
      // falls into below, so it covers every device type (Zigbee, Z-Wave,
      // Wi-Fi, etc.) — not just Z-Wave, which is all showZwaveBatteryDevices
      // covers (and only as "runs on battery", not "currently low"). Prefers
      // an actual percentage (measure_battery) when the device reports one;
      // falls back to a plain low-battery flag (alarm_battery) only for
      // devices with no percentage capability at all. class 'car' is
      // excluded — see the lowBatteryWarningPercent comment near the top.
      if (device.class !== 'car' && device.capabilitiesObj) {
        const battCap = device.capabilitiesObj.measure_battery;
        const battAlarm = device.capabilitiesObj.alarm_battery;
        if (battCap && typeof battCap.value === 'number' && battCap.value <= lowBatteryWarningPercent) {
          lowBatteryCount++; lowBatteryDevices.push(device.name);
        } else if (!battCap && battAlarm && battAlarm.value === true) {
          lowBatteryCount++; lowBatteryDevices.push(device.name);
        }
      }
      ...
      if (homeyPlatformVersion === 2 && driverId.includes('homey:virtualdrivergroup')) {
        ...
        zwaveNodeName[Number(device.settings.zw_node_id)] = device.name;
        zwaveById[device.id] = device.name;
    ...
    report.lowBattery = { total: lowBatteryCount, devices: lowBatteryDevices.sort() };
    if (showLowBatteryDevices) { log('---------------------------------------------'); log('Low battery devices (<= ' + lowBatteryWarningPercent + '%):'); log(lowBatteryDevices.join('\r\n')); log('---------------------------------------------'); }
    log(lowBatteryCount, 'devices with low battery (<= ' + lowBatteryWarningPercent + '%)');
...
await Homey.zwave.getState()
  .then(result => {
    ...
    // Look up the paired device's name for each unreachable node ID, via
    // zwaveNodeName (built above from device.settings.zw_node_id while
    // listing devices). This is reliable regardless of whether the node is
    // currently reachable: the name comes from Homey's own device registry,
    // not from live communication with the physical node, which is what
    // actually fails when a node is unreachable. Falls back to a bare
    // "Node X" only if no matching paired device exists (same fallback
    // "Unknown nodes" always uses, since those are defined as node IDs
    // with no matching paired device in the first place).
    const unreachableDisplay = unreachableSorted.map(id => zwaveNodeName[id] ? zwaveNodeName[id] + ' (Node ' + id + ')' : 'Node ' + id);
    ...
    report.zwave = {
      ...
      devicesById: zwaveById, // device.id -> name, added in v1.7.1 for rename detection
      ...
      unreachableNodes: unreachableDisplay,
      unreachableIds: unreachableSorted, // Z-Wave protocol node numbers, added in v1.7.2 for rename-proof tracking (unreachableDisplay embeds the name, so it isn't safe to diff on)
    ...
    if (showZwaveUnreachableNodes) { log('---------------------------------------------'); log('Unreachable nodes:'); log(unreachableDisplay.join('\r\n')); log('---------------------------------------------'); }
    if (showZwaveUnknownNodes) { log('---------------------------------------------'); log('Unknown nodes:'); log('Node ID:', unknownNodes.join('\r\nNode ID: ')); log('---------------------------------------------'); }

    log(zwave, 'paired Z-Wave devices', '(' + zwaveSx + ' Unsecure, ' + zwaveS0 + ' Secure (S0), ' + zwaveS2Auth + ' Secure (S2 Authenticated), ' + zwaveS2Unauth + ' Secure (S2 Unauthenticated), ' + zwaveRouter + ' router, ' + zwaveBattery + ' battery, ' + unreachableSorted.length + ' currently unreachable). ' + unknownNodes.length + ' unknown node(s) additionally found in the Z-Wave network with no matching paired device.')
  })
...
await Homey.zigbee.getState()
  .then(result => {
    ...
    // ieeeAddress -> name map, added in v1.7.1 for rename detection (same
    // pattern as virtualById/otherById/zwaveById) — confirmed live that the
    // object key here (below, `key`) IS the node's ieeeAddress, and that it
    // stays the same across an in-app rename (only .name changes).
    let zigbeeById = {};
    // Nodes that haven't reported in over zigbeeStaleDays, based on each
    // node's own lastSeen timestamp (not present on the coordinator entry,
    // i.e. Homey's own Zigbee radio — that one is simply skipped here,
    // same as it's implicitly skipped by not having a real router/enddevice
    // type above).
    let staleCount = 0, staleNodeNames = [], staleIds = [];
    const staleThresholdMs = zigbeeStaleDays * 24 * 3600 * 1000;
    const zigbeeNowMs = Date.now();

    Object.keys(result.nodes).forEach(function(key) {
      const node = result.nodes[key];
      zigbeeDevices.push(node.name);
      zigbeeById[key] = node.name;
      ...
      if (typeof node.lastSeen === 'number' && (zigbeeNowMs - node.lastSeen) > staleThresholdMs) {
        staleCount++; staleNodeNames.push(node.name); staleIds.push(key);
      }
    });
    ...
    report.zigbee = {
      ...
      nodesById: zigbeeById, // ieeeAddress -> name, added in v1.7.1 for rename detection
      ...
      stale: staleCount, staleNodes: staleNodeNames.sort(),
      staleIds // ieeeAddress list, added in v1.7.2 for rename-proof stale tracking
    };
    ...
    if (showZigbeeStaleNodes) { log('---------------------------------------------'); log('ZigBee nodes not responding (>' + zigbeeStaleDays + ' days):'); log(staleNodeNames.join('\r\n')); log('---------------------------------------------'); }

    allDevices += Object.keys(result.nodes).length;
    log(Object.keys(result.nodes).length, 'Zigbee nodes', '(' + router + ' router, ' + endDevice + ' end device, ' + unknownType + ' unknown type, ' + staleCount + ' not responding in >' + zigbeeStaleDays + ' days)');
  })
...
// Added in v1.7.2. Diffs SUBSET membership (e.g. "stale", "unreachable",
// "broken", "disabled") by ID rather than by name/display-string, so a
// same-run rename of the affected item doesn't produce a false
// "added"+"removed" pair — reliable because the ID stays the same across a
// rename, only the display changes. This is what closed the gap left by
// diffByIdWithRename() above: that function already made the population-
// level added/removed/renamed diff rename-proof, but a SUBSET check like
// "is this node currently stale" was still comparing name arrays, so a
// rename of an already-stale/unreachable item showed up as a false
// "recovered" + "newly affected" pair (confirmed live for both Zigbee
// stale-node tracking and, by the same underlying cause, Z-Wave
// unreachable-node tracking).
//   - prevIds / currIds: which IDs were/are in the subset this run.
//   - currAllIds: every ID that currently exists in the full population
//     (array) — used to tell "left the subset because it recovered" apart
//     from "left the subset because it was deleted" (a deletion is already
//     reported once, elsewhere, by the population-level diff, so it's
//     skipped here to avoid double-reporting).
//   - nameForId: function(id) -> display name, called only for ids that are
//     being reported (current data if the item still exists, since that
//     also correctly reflects a same-run rename).
//   - prevNamesFallback / currNamesFallback: the old subset NAME arrays,
//     used as a one-run fallback via diffLines() if no ID-based snapshot
//     exists yet (e.g. right after upgrading to this version).
function diffIdSubset(addedLabel, removedLabel, prevIds, currIds, currAllIds, nameForId, prevNamesFallback, currNamesFallback) {
  const out = [];
  if (Array.isArray(prevIds)) {
    const prevSet = new Set(prevIds);
    const currSet = new Set(currIds || []);
    const allIdSet = currAllIds ? new Set(currAllIds) : null;
    const addedIds = (currIds || []).filter(id => !prevSet.has(id));
    const removedIds = prevIds.filter(id => !currSet.has(id) && (!allIdSet || allIdSet.has(id)));
    if (addedIds.length > 0) out.push({ tag: 'added', label: addedLabel, count: addedIds.length, names: addedIds.map(nameForId) });
    if (removedIds.length > 0) out.push({ tag: 'removed', label: removedLabel, count: removedIds.length, names: removedIds.map(nameForId) });
  } else {
    // No ID-based snapshot yet — one-run fallback to the old name-only diff.
    return diffLines(addedLabel, removedLabel, undefined, undefined, prevNamesFallback, currNamesFallback);
  }
  return out;
}
...
if (previousSnapshot) {
  ...
  // Unexpected restart: uptime can only increase while Homey keeps running,
  // so a CURRENT uptime that's smaller than what was recorded last time
  // means it restarted at some point in between — true regardless of how
  // long ago the previous scan ran, so there's no need to also track when
  // that was. Skipped when a firmware update also happened this run (see
  // just above) since that already explains + implies a restart on its own.
  if (
    previousSnapshot.uptimeSeconds !== undefined && report.uptimeSeconds !== undefined
    && report.uptimeSeconds < previousSnapshot.uptimeSeconds
    && previousSnapshot.homeyVersion === report.version
  ) {
    findings.push({ level: 'warn', text: '🔄 Homey restarted since the previous scan (uptime reset — not caused by a firmware update, see above).' });
  }
  ...
  // Basic flows broken/disabled: ID-based since v1.7.2 (diffIdSubset),
  // fully independent of the rename diff below — so a flow that's both
  // renamed AND newly broken/disabled in the same run correctly produces
  // two separate findings, always shown under its CURRENT name. Falls back
  // to the old name-only diffLinesEx for one run if no ID-based broken/
  // disabled snapshot exists yet (e.g. right after upgrading).
  diffIdSubset('🔀 Basic flows broken', '🔀 Basic flows fixed (no longer broken)', previousSnapshot.flowsBrokenIds, report.flows.brokenIds, Object.keys(report.flows.allById), id => report.flows.allById[id] || id, previousSnapshot.flowsBrokenNames, report.flows.brokenNames)
    .forEach(e => findings.push({ level: (e.tag === 'added' && report.flows.broken > (previousSnapshot.flowsBroken || 0)) ? 'crit' : 'info', ...e }));
  diffIdSubset('🔀 Basic flows disabled', '🔀 Basic flows re-enabled', previousSnapshot.flowsDisabledIds, report.flows.disabledIds, Object.keys(report.flows.allById), id => report.flows.allById[id] || id, previousSnapshot.flowsDisabledNames, report.flows.disabledNames)
    .forEach(e => findings.push({ level: 'info', ...e }));

  // Basic flows created/deleted/renamed: ID-based since v1.7.2
  // (diffByIdWithRename, same pattern as devices) — a renamed flow is now
  // reported as "renamed" instead of a remove+add pair. Falls back to the
  // old name-only diff for one run if no ID-based snapshot exists yet.
  diffByIdWithRename('🔀 Basic flows created', '🔀 Basic flows deleted', '🔀 Basic flows renamed', previousSnapshot.flowsAllById, report.flows.allById, previousSnapshot.flowsAllNames, report.flows.allNames)
    .forEach(e => findings.push({ level: 'info', ...e }));

  // Advanced flows (same logic as Basic flows above)
  diffIdSubset('🔀 Advanced flows broken', '🔀 Advanced flows fixed (no longer broken)', previousSnapshot.advFlowsBrokenIds, report.advancedFlows.brokenIds, Object.keys(report.advancedFlows.allById), id => report.advancedFlows.allById[id] || id, previousSnapshot.advFlowsBrokenNames, report.advancedFlows.brokenNames)
    .forEach(e => findings.push({ level: (e.tag === 'added' && report.advancedFlows.broken > (previousSnapshot.advFlowsBroken || 0)) ? 'crit' : 'info', ...e }));
  diffIdSubset('🔀 Advanced flows disabled', '🔀 Advanced flows re-enabled', previousSnapshot.advFlowsDisabledIds, report.advancedFlows.disabledIds, Object.keys(report.advancedFlows.allById), id => report.advancedFlows.allById[id] || id, previousSnapshot.advFlowsDisabledNames, report.advancedFlows.disabledNames)
    .forEach(e => findings.push({ level: 'info', ...e }));

  // Advanced flows created/deleted/renamed: same as Basic flows above.
  diffByIdWithRename('🔀 Advanced flows created', '🔀 Advanced flows deleted', '🔀 Advanced flows renamed', previousSnapshot.advFlowsAllById, report.advancedFlows.allById, previousSnapshot.advFlowsAllNames, report.advancedFlows.allNames)
    .forEach(e => findings.push({ level: 'info', ...e }));
  ...
  // Low battery: same "newly in the list / no longer in the list" diff
  // pattern as Z-Wave's unreachable-nodes check below (diffLines, not
  // diffLinesEx) — a device leaving the list either means its battery
  // was replaced/recharged, or the device itself was removed; either way
  // reads fine as "no longer low on battery" here, consistent with how
  // "reachable again" is used for Z-Wave nodes even if one was actually
  // deleted rather than fixed.
  if (report.lowBattery) {
    diffLines('🔋 Devices with low battery', '🔋 Devices no longer low on battery', previousSnapshot.lowBatteryTotal, report.lowBattery.total, previousSnapshot.lowBatteryDeviceNames, report.lowBattery.devices)
      .forEach(e => findings.push({ level: e.tag === 'added' ? 'warn' : 'info', ...e }));
  }

  if (report.zwave && !report.zwave.failed) {
    diffByIdWithRename('📡 Z-Wave devices added', '📡 Z-Wave devices removed', '📡 Z-Wave devices renamed', previousSnapshot.zwaveDeviceById, report.zwave.devicesById, previousSnapshot.zwaveDeviceNames, report.zwave.devices)
      .forEach(e => findings.push({ level: 'info', ...e }));
    // Unreachable/unknown-type nodes now list WHICH node IDs, not just a bare
    // count (unreachableNodes also includes the paired device's name where
    // one is known, e.g. "Lamp boom (Node 39)" — see the Z-Wave section
    // above), independent of the showZwave...Nodes display toggles, so the
    // diff still works correctly even if you've turned the on-screen list
    // off.
    // ID-based since v1.7.2 (diffIdSubset), on the Z-Wave protocol node
    // number (unreachableIds) rather than the composed display string
    // (unreachableNodes, which embeds the device name and so isn't safe to
    // diff on) — a rename of an already-unreachable device no longer
    // produces a false "newly unreachable" + "reachable again" pair.
    diffIdSubset('📡 Z-Wave nodes newly unreachable', '📡 Z-Wave nodes reachable again', previousSnapshot.zwaveUnreachableIds, report.zwave.unreachableIds, zwaveNodes, id => (zwaveNodeName[id] ? zwaveNodeName[id] + ' (Node ' + id + ')' : 'Node ' + id), previousSnapshot.zwaveUnreachableNodes, report.zwave.unreachableNodes)
      .forEach(e => findings.push({ level: e.tag === 'added' ? 'warn' : 'info', ...e }));
  ...
  if (report.zigbee && !report.zigbee.failed) {
    diffByIdWithRename('📶 Zigbee devices added', '📶 Zigbee devices removed', '📶 Zigbee devices renamed', previousSnapshot.zigbeeNodeById, report.zigbee.nodesById, previousSnapshot.zigbeeNodeNames, report.zigbee.nodes)
      .forEach(e => findings.push({ level: 'info', ...e }));
    // Same diffLines pattern as Z-Wave's unreachable-nodes check above —
    // the Zigbee equivalent, based on each node's own lastSeen timestamp
    // (see zigbeeStaleDays near the top of the script).
    // ID-based since v1.7.2 (diffIdSubset), on ieeeAddress rather than name
    // — a rename of an already-stale node no longer produces a false
    // "newly not responding" + "reporting again" pair (confirmed live:
    // renaming a 20-day-stale node produced exactly that false pair before
    // this fix).
    diffIdSubset('📶 Zigbee nodes newly not responding (>' + zigbeeStaleDays + ' days)', '📶 Zigbee nodes reporting again', previousSnapshot.zigbeeStaleIds, report.zigbee.staleIds, Object.keys(report.zigbee.nodesById), id => report.zigbee.nodesById[id] || id, previousSnapshot.zigbeeStaleNodes, report.zigbee.staleNodes)
      .forEach(e => findings.push({ level: e.tag === 'added' ? 'warn' : 'info', ...e }));
  }
} else {
...
const snapshot = {
  ...
  flowsAllById: report.flows.allById, flowsBrokenIds: report.flows.brokenIds, flowsDisabledIds: report.flows.disabledIds,
  ...
  advFlowsAllById: report.advancedFlows.allById, advFlowsBrokenIds: report.advancedFlows.brokenIds, advFlowsDisabledIds: report.advancedFlows.disabledIds,
  ...
  lowBatteryTotal: report.lowBattery ? report.lowBattery.total : undefined,
  lowBatteryDeviceNames: report.lowBattery ? report.lowBattery.devices : undefined,
  ...
  zwaveDeviceById: (report.zwave && !report.zwave.failed) ? report.zwave.devicesById : undefined,
  ...
  zwaveUnreachableIds: (report.zwave && !report.zwave.failed) ? report.zwave.unreachableIds : undefined,
  ...
  zigbeeNodeById: (report.zigbee && !report.zigbee.failed) ? report.zigbee.nodesById : undefined,
  zigbeeStale: (report.zigbee && !report.zigbee.failed) ? report.zigbee.stale : undefined,
  zigbeeStaleNodes: (report.zigbee && !report.zigbee.failed) ? report.zigbee.staleNodes : undefined,
  zigbeeStaleIds: (report.zigbee && !report.zigbee.failed) ? report.zigbee.staleIds : undefined
};
...
// Like findingsHtml(), but renders the same findings as plain text instead
// of HTML, for the optional WhatsApp summary (see sendWhatsAppSummary near
// the top of the script). No HTML tags/escaping — f.names are used as-is,
// since they're plain strings here, not embedded in markup.
function findingsText(list) {
  if (list.length === 0) return '✅ No notable changes since the previous scan.';
  return list.map(f => {
    if (f.names && f.names.length > 0) {
      const namesList = f.names.map(n => '  - ' + n).join('\n');
      let inner = `${f.count} ${f.label} since previous scan:\n${namesList}`;
      if (f.note) inner += `\n  (${f.note})`;
      return inner;
    } else if (f.countOnly) {
      return `${f.label}: ${f.countOnly}`;
    } else {
      return f.text;
    }
  }).join('\n\n');
...
if (includeDetails) {
  ...
  sections.push({
    ...
    html: `
      ...
      ${showStableApps ? '<h3>Stable channel apps</h3>' + ul(report.apps.stableApps) : ''}
  ...
  sections.push({
    ...
    html: `
      <table class="kv">
        ...
        <tr><th>Low battery devices (&le;${lowBatteryWarningPercent}%)</th><td>${report.lowBattery ? report.lowBattery.total : 0}</td></tr>
      </table>
      ...
      ${showIRDevices ? '<h3>Infrared devices</h3>' + ul(report.devices.irNames) : ''}
      ...
      ${showLowBatteryDevices ? '<h3>Low battery devices</h3>' + ul(report.lowBattery ? report.lowBattery.devices : []) : ''}
    `
  });
  ...
  if (report.zwave && !report.zwave.failed) {
    sections.push({
      ...
      html: `
        <p><strong>${report.zwave.total}</strong> paired Z-Wave devices — ${report.zwave.unsecure} unsecure, ${report.zwave.s0} S0, ${report.zwave.s2auth} S2 (auth), ${report.zwave.s2unauth} S2 (unauth), ${report.zwave.router} router, ${report.zwave.battery} battery, ${report.zwave.unreachable} currently unreachable.</p>
        <p>Additionally, <strong>${report.zwave.unknown}</strong> unknown node(s) found in the Z-Wave network with no matching paired device.</p>
        ${showZwaveUnsecureDevices ? '<h3>Unsecure devices</h3>' + ul(report.zwave.unsecureDevices) : ''}
        ${showZwaveSecureS0Devices ? '<h3>Secure (S0) devices</h3>' + ul(report.zwave.s0Devices) : ''}
        ${showZwaveSecureS2AuthenticatedDevices ? '<h3>Secure (S2 Authenticated) devices</h3>' + ul(report.zwave.s2authDevices) : ''}
        ${showZwaveSecureS2UnauthenticatedDevices ? '<h3>Secure (S2 Unauthenticated) devices</h3>' + ul(report.zwave.s2unauthDevices) : ''}
        ${showZwaveRouterDevices ? '<h3>Router devices</h3>' + ul(report.zwave.routerDevices) : ''}
        ${showZwaveBatteryDevices ? '<h3>Battery devices</h3>' + ul(report.zwave.batteryDevices) : ''}
        ${showZwaveUnreachableNodes ? '<h3>Unreachable nodes</h3>' + ul(report.zwave.unreachableNodes) : ''}
  ...
  if (report.zigbee && !report.zigbee.failed) {
    sections.push({
      ...
      html: `
        <p><strong>${report.zigbee.total}</strong> nodes — ${report.zigbee.router} router, ${report.zigbee.endDevice} end device, ${report.zigbee.unknownType} unknown type, ${report.zigbee.stale} not responding in &gt;${zigbeeStaleDays} days.</p>
        ${showZigbeeRouter ? '<h3>Router devices</h3>' + ul(report.zigbee.routerDevices) : ''}
        ${showZigbeeEndDevice ? '<h3>End devices</h3>' + ul(report.zigbee.endDevices) : ''}
        ${showZigbeeStaleNodes ? '<h3>Not responding (&gt;' + zigbeeStaleDays + ' days)</h3>' + ul(report.zigbee.staleNodes) : ''}
...
<div style="font-family:Segoe UI,Arial,sans-serif;max-width:640px;margin:0 auto;color:#222;">
  <style>
    ...
    .footer { color:#999; font-size:10px; font-style:italic; margin-top:30px; border-top:1px solid #eee; padding-top:10px; }
  </style>
  ...
  <p class="footer">Automatically generated by HomeyScript ${esc(SCRIPT_NAME)} v${esc(VERSION)}.</p>
</div>
`;
...
let resultMsg = failed.length > 0
  ? 'Overview finished. Sent to ' + (sent.join(', ') || '(none)') + '. FAILED for: '
    + failed.map(f => f.addr + ' (' + f.error + ')').join('; ')
  : 'Overview finished, email sent to ' + sent.join(', ');

// Optional WhatsApp summary via the CallMeBot Homey app (see
// sendWhatsAppSummary near the top). Sent unconditionally on every run,
// independent of onlyShowDetailsOnChange — it's always just the short
// Summary text, never the full detail report, so there's no "detail" to
// gate. Calls the CallMeBot app's own "Send a message" flow action, same
// pattern as the email send above — no phone number or API key in this
// script itself. Errors are caught and logged the same way as the email
// send, so a CallMeBot hiccup doesn't fail the whole script.
if (sendWhatsAppSummary) {
  const whatsappText = `🏠 Homey Overview — ${dateStr}\n\n${findingsText(findings)}`;
  try {
    // Find the CallMeBot WhatsApp device (added via the CallMeBot app,
    // com.gruijter.callmebot) by its driver ID — works regardless of what
    // you named the device or which WhatsApp number it's registered to.
    const devicesForWa = await Homey.devices.getDevices();
    const waDevice = Object.values(devicesForWa).find(d => d.driverId === 'homey:app:com.gruijter.callmebot:whatsapp');
    if (!waDevice) {
      throw new Error('No CallMeBot WhatsApp device found — install the CallMeBot app and add a WhatsApp device first (see README).');
    }
    await Homey.flow.runFlowCardAction({
      id: `homey:device:${waDevice.id}:send`,
      args: { msg: whatsappText }
    });
    log('WhatsApp summary sent via CallMeBot device:', waDevice.name);
    resultMsg += '. WhatsApp summary sent.';
  } catch (e) {
    const msg = (e && e.message) ? e.message : String(e);
    log('Failed to send WhatsApp summary via CallMeBot —', msg);
    resultMsg += '. WhatsApp summary FAILED: ' + msg;
  }
}
